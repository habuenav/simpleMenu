...
class sm
{
    ...
    txtMenu *_opt=NULL; // opciones en formato string
    ...
    txtMenu *_root_opt=NULL; // almacena el menu principal establecido con el metodo Root
...
if(_tTime > (_timeout * 1000) && _timeout>0) {disp.clearBuffer();}
disp.sendBuffer();
}
...
_opt = new String[size(opt)]; 
for (uint8_t i = 0; i < size(opt); i++) 
{
_opt[i]=opt[i]; 
...
//Serial.println(_titleht);
//Serial.println(_lineht);
//Serial.println(_interline);
//Serial.println(_max_lines);
...
template <typename T, class tipo> 
void sm::Root(T (&opt), tipo _id,bool title)
{
String test=String(_id);

_root_opt = new String[size(opt)]; 
for (uint8_t i = 0; i < size(opt); i++) {_root_opt[i]=opt[i]; }
